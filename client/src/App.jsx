...
const App = () => {
  ...
  const captureEmotion = async (historyRef) => {
    ...
    try {
      const response = await  fetch('/.netlify/functions/detect_emotion', {
        method: 'POST',
  ...
  const startCapture = async () => {
    ...
    setTimeout(() => {
      ...
      console.log('📊 Final emotion tally:', emotionCounts);
  ...
  const fetchAIYogaSuggestions = async (emotion) => {
    ...
    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer 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`,
  ...
  const fetchYoutubeVideos = async (emotion) => {
    ...
    try {
      const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&type=video&maxResults=2&key=AIzaSyDfDpqa3y3XxNwdiKJV4lPnKNvSfH51E10`);
      const json = await res.json();
  ...
  return (
    <div className="app">
      <div className="main-content">
        <h1 className="title">MindMirror</h1>
